function val = negLogLikSpaceTimeExpGeom_Sunke_new(params,interpLatAggr,interpLongAggr,interpJulDayAggr,interpResAggr,interpErrorAggr)
...
for iYear = 1:nYear
    ...
    interpErrorYear = interpErrorAggr{iYear};
    ...
    val = val + sum(log(eig(covObs + diag(sigma^2+interpErrorYear.^2) ))) + (interpResYear)'*((covObs + diag(sigma^2+interpErrorYear.^2))\interpResYear) + log(2*pi)*nRes;
    %toc;
